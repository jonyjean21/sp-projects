...
#### 記憶の蓄積（セッション中に随時）
- **セッションログ**: `sp-brain/memory/sessions/YYYYMMDD.md` に **大きな作業が完了するたびに追記する**（セッション終了を待たない）
  - フックがセッション開始時刻を自動記録する
  - 主要タスク完了後、`### HH:MM やったこと` の形式で追記する（Editツールで末尾追記）
  - セッションがクラッシュしても開始記録は残る

...
| クラウドワークス受託 | ¥0 | ¥50,000 |
| **Claude Code 発信（X + note）★新規** | **¥0** | **¥15,000** |
| X アフィリエイト | ¥0 | ¥5,000 |
| その他 | ¥0 | ¥10,000 |
| **合計** | **¥3,000** | **¥155,000** |
...
## ⭐ 新戦略: Claude Code 発信（ブルーオーシャン）

**なぜ狙うか**: Claude Codeのリアルな使い方を発信してる日本人がほぼゼロ。
SPは実際に使って稼いでいる当事者 → 信頼性が高く競合がいない。

**収益の流れ**:
```
X で Claude Code 解説投稿（週2回・無料）
  ↓ AI副業に興味ある層がフォロー
note 有料マガジンへ誘導（¥1,980/月）
  ↓
「Claude Code使えます」でクラウドワークス差別化
  ↓ 受託単価が上がる
```

**X 投稿テーマ例**（全部実体験）:
- ターミナル版 vs ブラウザ版の違い
- SESSION-HANDOFF.md でAIに引き継がせる仕組み
- cronで全自動化する方法
- API費用と収益を正直に公開

**生成コマンド**:
```bash
node income/06-x-auto-post/x-affiliate-poster.js --type=claudecode --dry-run
```

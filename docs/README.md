# ドキュメント索引

意思決定ログ、設計メモ、ナレッジをここに蓄積する。

## 設計メモ

| ドキュメント | 内容 | 作成日 |
|-------------|------|--------|
| [寄稿フロー設計](contribution-flow-design.md) | 寄稿受付の統一 → GAS連携 → 半自動化の3段階プラン | 2026-02-24 |
| [大会情報収集の自動化](tournament-collection-design.md) | Google Alerts + IFTTT + 手動の3層パイプライン設計 | 2026-02-24 |
| [記事半自動化](article-automation-design.md) | 定型/半定型/非定型の記事タイプ別フロー設計 | 2026-02-24 |
| [SEO改善プラン](seo-improvement-plan.md) | SWELL最適化・優先施策TOP3・月次ルーチン | 2026-02-24 |
| [GA自動エクスポート](ga-auto-export-design.md) | GA4データをGASで月次自動エクスポートする設計 | 2026-02-24 |

## マルタ村関連

| ドキュメント | 内容 | 作成日 |
|-------------|------|--------|
| [コードレビュー・ワークフロー](marutamura-review-workflow.md) | 中さん→GitHub→Claude レビュー→Issue→再デプロイのサイクル設計 | 2026-02-24 |
| [マルタ村 全体設計](marutamura-architecture.md) | アプリ間接続・共通基盤・レイヤー構造・今後の論点整理 | 2026-02-24 |

## 意思決定ログ

重要な技術選定や方針決定の記録。

*(今後追加)*

## ナレッジ

学んだこと、調査結果、参考情報。

*(今後追加)*
